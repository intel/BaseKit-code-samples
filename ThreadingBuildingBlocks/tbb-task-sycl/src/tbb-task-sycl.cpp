...
class execute_on_gpu {
  ...
  { //starting SYCL code
    ...
    device_queue.submit([&](cl::sycl::handler & cgh) {
      auto A_accessor = A_array_buffer.get_access <cl::sycl::access::mode::read> (cgh);
      auto B_accessor = B_array_buffer.get_access <cl::sycl::access::mode::read> (cgh);
      auto C_accessor = C_array_buffer.get_access <cl::sycl::access::mode::write> (cgh);
...
int main(){
  ...
  if (!std::equal(std::begin(C_array_tbb), std::end(C_array_tbb), std::begin(CGold)))
