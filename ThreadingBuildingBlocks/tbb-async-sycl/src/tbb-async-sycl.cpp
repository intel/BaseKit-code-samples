...
void print_array(const char * text,
  const std::array<float, ARRAY_SIZE> & array){
  std::cout << text;
...
class AsyncActivity {
  ...
  void run(float offload_ratio, gateway_type & gateway) {
    ...
    a.enqueue([&, offload_ratio]() {
      ...
      std::cout << "start index for GPU = 0; end index for GPU = " << array_size_sycl << std::endl;
      ...
      { //starting SYCL code
        ...
        cl::sycl::buffer<cl::sycl::cl_float, 1> A_array_buffer(A_array.data(), n_items);
        cl::sycl::buffer<cl::sycl::cl_float, 1> B_array_buffer(B_array.data(), n_items);
        cl::sycl::buffer<cl::sycl::cl_float, 1> C_array_buffer(C_array.data(), n_items);
...
int main() {
  ...
  tbb::flow::graph g;
  // Associate the graph with the main arena
  main_arena.execute([&g] { g.reset(); });

  //Source node:
  bool n = false;
  tbb::flow::source_node<float> in_node {
    g,
    [&](float & offload_ratio) {
      if (n) return false;
      offload_ratio = ratio;
  ...
  tbb::flow::function_node<float, double> cpu_node{
    ...
    [&](float offload_ratio) -> double {
      ...
      std::cout << "start index for CPU = " << i_start << "; end index for CPU = " << i_end << std::endl;
