...
#define POW \
  (20)  // Kernel executes a power function (base^POW). Must be >= 2. Can
        // increase this to increase kernel execution time, but process_output()
...
void simple_pow(std::unique_ptr<queue> &deviceQueue,
  ...
  update_host_event = deviceQueue->submit([&](handler &cgh) {
    ...
    /*
      ...
      functionality since the buffer access in process_output() also implicitly
      instructs the runtime to copy the data back. But it should be noted that
      this buffer access blocks process_output() until the kernel is complete
      and the data is copied. In contrast, update_host() instructs the runtime
      to perform the copy earlier. This allows process_output() to optionally
      perform more useful work *before* making the blocking buffer access. Said
      another way, this allows process_output() to potentially perform more work
...
/*  Compares kernel output against expected output.
   ...
   place to do this because process_output() is blocked on kernel completion.
*/
void process_output(buffer<cl_float, 1> &output_buf,
                    std::vector<cl_float> &input_copy, int exec_number,
  ...
  for (int i = 0; i < SIZE; i++) {
    if ((num_errors < num_errors_to_print) &&
        (my_pow(input_copy.data()[i], POW) != output_buf_acc[i])) {
      if (num_errors == 0) {
        pass = false;
      ...
      std::cout << "Verification failed on kernel execution # " << exec_number
                ...
                << std::setprecision(16) << my_pow(input_copy.data()[i], POW)
...
int main() {
  try {
    ...
    std::vector<cl_float> input_buf_copy
        [2 * LOCAL_N];  // For every execution slot, we need 2 host-side buffers
    ...
    cl_ulong total_kernel_time_per_slot[LOCAL_N];  // In nanoseconds. Total
                                                   // execution time of kernels
                                                   // in a given slot.
    cl_ulong total_kernel_time = 0;  // Total execution time of all kernels.
    std::thread t_process_output[LOCAL_N];  // Threads to process the output
                                            // from each kernel
    std::thread t_process_input[LOCAL_N];   // Threads to process the input data
                                            // for the next kernel
    ...
    */
    for (int i = 0; i < NUM_RUNS; i++) {
      ...
      switch (i) {
        ...
        case 3: {
          std::cout << "*** Beginning execution, N=" << LOCAL_N
      ...
      for (int i = 0; i < N; i++) {
        t_process_input[i] = std::thread(process_input, std::ref(input_buf[i]),
      ...
      */
      for (int i = 0; i < TIMES; i++) {
        ...
        // Before each kernel launch, the process_output() must have completed
        // for the last execution in this slot. The process_input() must also
        // have completed for the upcoming new execution for this slot. Block on
        // both of these.
        if (!st) {
          if (i >= N) {  // process_output() is only relevant after the first N
        ...
        simple_pow(device_queue, input_buf[i % N], output_buf[i % N],
                   sycl_events[i % N]);

        // Immediately launch threads for the process_output() and
        // process_input() for *this* slot. These are non-blocking with respect
        // to main(), but they will individually be blocked until the
        // corresponding kernel execution is complete. The process_output()
        // compares the kernel output data against the input data. But
        // process_input() will be overwriting that input data in parallel.
        // Therefore process_output() must compare against an older copy of the
        // data. We ping-pong between host-side copies of the input data.
        t_process_output[i % N] = std::thread(
            process_output, std::ref(output_buf[i % N]),
        ...
        if (i <
            TIMES - N) {  // For the final N kernel launches, no need to process
                          // input data because there will be no more launches.
          t_process_input[i % N] = std::thread(
              process_input, std::ref(input_buf[i % N]),
              std::ref(
                  input_buf_copy[(i + N) %
                                 (2 *
                                  N)]));  // The indexes for the input_buf_copy
                                          // used by process_output() and
                                          // process_input() are spaced N apart.
          if (st) {
            t_process_input[i % N].join();
          }
        }
      ...
      std::cout << std::endl
                ...
                << ((i == NUM_RUNS - 1) ? ("with N-way buffering ") : "")
