...
#define NUM_RUNS \
  (2)  // This tutorial runs twice to show the impact with and without the
       // optimization.
...
#define SIZE \
  (8 * 1024)  // Number of inputs. Don't set this too large, otherwise
              ...
              // the host (the time is proportional to SIZE^2)
#define M \
...
void triangular_loop(std::unique_ptr<queue>& device_queue,
                     buffer<uint32_t>& input_buf, buffer<uint32_t>& output_buf,
                     uint32_t n, event& queue_event, bool optimize) {
  // Enqueue kernel
  queue_event = device_queue->submit([&](cl::sycl::handler& cgh) {
    ...
    auto _output =
        output_buf.get_access<cl::sycl::access::mode::discard_write>(cgh);
    ...
    cgh.single_task<Task>([=]() {
      ...
      if (!optimize) {  // Unoptimized loop.
        ...
        // variables is M iterations. We ensure this is true by modifying the y
        // index such that a minimum of M iterations are always executed.
        [[intelfpga::ivdep(M)]] for (int i = 0; i < loop_bound; i++) {
          ...
          y++;
          if (y == n) {  // If we've hit the end, set y such that a minimum of M
...
// Create queue, get platform and device
    ...
    for (int i = 0; i < NUM_RUNS; i++) {
      switch (i) {
        case 0: {
          std::cout << "Beginning run without triangular loop optimization."
                    << std::endl
                    << std::endl;
          triangular_loop(device_queue, input_buf, output_buf, SIZE,
                          queue_event, false);
          break;
        }
        case 1: {
      ...
      t1_kernel = queue_event.template get_profiling_info<
          info::event_profiling::command_start>();
      t2_kernel = queue_event.template get_profiling_info<
