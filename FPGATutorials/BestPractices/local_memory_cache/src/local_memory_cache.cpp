...
void histogram(std::unique_ptr<queue>& device_queue,
               ...
               event& queue_event, bool use_cache) {
  // Enqueue  kernel
    ...
    cgh.single_task<Task>([=]() {
      ...
      if (!cache) {  // Without cache
        for (uint32_t n = 0; n < INIT_NUM_INPUTS; ++n) {
          uint32_t b = _input[n] %
                       NUM_OUTPUTS;  // Compute the histogram index to increment
          local_output[b]++;
        }
      } else {  // With cache
        [[intelfpga::ivdep(
            CACHE_DEPTH)]]  // Specify that the minimum dependence-distance of
                            // loop carried variables is CACHE_DEPTH.
                            for (uint32_t n = 0; n < INIT_NUM_INPUTS; ++n) {
          uint32_t b = _input[n] %
                       NUM_OUTPUTS;  // Compute the histogram index to increment
...
// Create queue, get platform and device
    ...
    std::cout << std::endl
              << "Number of inputs: " << INIT_NUM_INPUTS << std::endl;
    ...
    for (int i = 0; i < NUM_RUNS; i++) {
      switch (i) {
        ...
        case 1: {
          std::cout << "Beginning run with local memory caching." << std::endl
                    << std::endl;
      ...
      if (passed) {
        ...
        double N_MB = (INIT_NUM_INPUTS * sizeof(uint32_t)) /
        ...
        std::cout << "Kernel throughput " << (i == 0 ? "without" : "with")
                  << " caching: " << N_MB / time_kernel << " MB/s" << std::endl
                  << std::endl;
      } else {
