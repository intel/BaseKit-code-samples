...
void multiply2(int msize, int tidx, int numt, TYPE a[][NUM], TYPE b[][NUM],
               TYPE c[][NUM], TYPE t[][NUM]) {
  int i, j, k;

  // Step 2: Loop interchange
  // Add compile option for vectorization report Windows: /Qvec-report3 Linux
  // -vec-report3
  for (i = tidx; i < msize; i = i + numt) {
    for (k = 0; k < msize; k++) {
#pragma ivdep
...
void multiply3(int msize, int tidx, int numt, TYPE a[][NUM], TYPE b[][NUM],
               TYPE c[][NUM], TYPE t[][NUM]) {
  ...
  // Define the ALIGNED in the preprocessor definitions and compile option
  // Windows: /Oa Linux: -fno-alias
  istep = msize / numt;
  ibeg = tidx * istep;
  ibound = ibeg + istep;
...
void multiply4(int msize, int tidx, int numt, TYPE a[][NUM], TYPE b[][NUM],
               TYPE c[][NUM], TYPE t[][NUM]) {
...
void multiply1(int msize, int tidx, int numt, TYPE a[][NUM], TYPE b[][NUM],
  ...
  cl::sycl::queue deviceQueue(device, exception_handler);
  ...
  deviceQueue
    ...
    cgh.parallel_for<class Matrix1<TYPE>>(matrix_range,
      ...
      for (k = 0; k < NUM; k++) {
        ...
        // accessorC[ind[0]][ind[1]]  += accessorA[ind[0]][k] *
        // accessorB[k][ind[1]];
      }
...
void multiply1_2(int msize, int tidx, int numt, TYPE a[][NUM], TYPE b[][NUM],
  ...
  deviceQueue
      .submit([&](cl::sycl::handler& cgh) {
        ...
        // Create matrix tiles
        cl::sycl::accessor<TYPE, 2, cl::sycl::access::mode::read_write,
...
void multiply2(int msize, int tidx, int numt, TYPE a[][NUM], TYPE b[][NUM],
  ...
  for (i0 = 0; i0 < msize; i0 += mblock) {
    for (j0 = 0; j0 < msize; j0 += mblock) {
      for (k0 = 0; k0 < msize; k0 += mblock) {
        cl::sycl::buffer<TYPE, 2> subbufa(
            bufferA, cl::sycl::id<2>(i0, k0),
            cl::sycl::range<2>(MATRIX_BLOCK_SIZE, MATRIX_BLOCK_SIZE));
        cl::sycl::buffer<TYPE, 2> subbufb(
            bufferB, cl::sycl::id<2>(k0, j0),
            cl::sycl::range<2>(MATRIX_BLOCK_SIZE, MATRIX_BLOCK_SIZE));
        cl::sycl::buffer<TYPE, 2> subbufc(
            bufferC, cl::sycl::id<2>(i0, j0),
            cl::sycl::range<2>(MATRIX_BLOCK_SIZE, MATRIX_BLOCK_SIZE));
...
// Cache-blocked matrix multiply using sub-ranges. Also replaces accessorC
// reference with a local variable
void multiply2_1(int msize, int tidx, int numt, TYPE a[][NUM], TYPE b[][NUM],
                 TYPE c[][NUM], TYPE t[][NUM]) {
  int i, j, k, i0, j0, k0, ibeg, ibound, istep, mblock;
  // Select device
  //#ifdef GPU
...
// Link with Intel MKL library: With MSFT VS and Intel Composer integration:
// Select build components in the Project context menu. For command line - check
// out the Intel Math Kernel Library Link Line Advisor
void multiply5(int msize, int tidx, int numt, TYPE a[][NUM], TYPE b[][NUM],
               TYPE c[][NUM], TYPE t[][NUM]) {
  double alpha = 1.0, beta = 0.;
  cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, NUM, NUM, NUM, alpha,
              (const double*)b, NUM, (const double*)a, NUM, beta, (double*)c,
              NUM);
}
#endif  // USE_MKL
