...
void vec_add(const std::vector<float>& VA, const std::vector<float>& VB, std::vector<float>& VC, int n) {
  ...
  try{
    ...
    try {
      deviceQueue.reset( new queue(device_selector, property_list) );
    ...
    cl_ulong startk = queue_event.template get_profiling_info<cl::sycl::info::event_profiling::command_start>();
    cl_ulong endk = queue_event.template get_profiling_info<cl::sycl::info::event_profiling::command_end>();
    // unit of startk and endk is nano second, convert to ms
    double kernel_time = (double)(endk - startk) * 1e-6f;

    std::cout << "UNROLL_FACTOR " << UNROLL_FACTOR <<  "kernel time : " << kernel_time << " ms\n";
    std::cout << "Throughput for kernel with UNROLL_FACTOR " << UNROLL_FACTOR << ": ";
    std::cout << std::fixed << std::setprecision(3) << ((double) n / kernel_time) / 1e6f << " GFlops\n";
  } catch (cl::sycl::exception const& e) {
...
int main(int argc, char* argv[]) {
  ...
  for (unsigned int i = 0; i < array_size; i++) {
    if (C1[i] != A[i] + B[i] ||
        C1[i] != C2[i] || C1[i] != C3[i] || C1[i] != C4[i] || C1[i] != C5[i]) {
