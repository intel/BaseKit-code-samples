...
#define TIMES     (3)         // # times to execute the kernel. TIMES must be >= 2; small for this tutorial for easier visualization of data.
...
/*  Note that this is the same design as that of the double_buffering tutorial. 
    This tutorial uses the OpenCL Intercept Layer to visualize the same double_buffering optimization.
*/
...
*/
void simple_pow ( queue &deviceQueue,
...
int main() {
  ...
  try {
    ...
    try {
      device_queue.reset( new queue(device_selector, property_list) );
    } catch (cl::sycl::exception const& e) {
    ...
    */
    for (int i=0;i<NUM_RUNS;i++) {
      ...
      std::cout << std::endl << "Overall execution time = " << (unsigned) (time_span.count() * 1000) << " ms" << std::endl;
      std::cout << "Total kernel-only execution time = " << (unsigned) (total_kernel_time / 1000000) << " ms" << std::endl;
