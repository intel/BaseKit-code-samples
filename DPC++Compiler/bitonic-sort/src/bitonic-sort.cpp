...
void SwapElements(int step, int stage, int num_sequence, int seq_len,
  ...
  for (int seq_num = 0; seq_num < num_sequence; seq_num++) {
    ...
    for (int i = seq_num * seq_len; i < seq_num * seq_len + h_len;
         i++) {
...
void Usage(std::string prog_name, int exponent) {
  ...
  std::cout << " k: Seek used to generate a random sequence.\n";
}
...
int main(int argc, char *argv[]) {
  ...
  // numbers.
  int *data = (int *)malloc_shared(size * sizeof(int), q);

  // Memory allocated for host access only.
  int *data2 = (int *)malloc(size * sizeof(int));

  // Initialize the array randomly using a seek.
  srand(seek);
  ...
  for (int i = 0; i < size - 1; i++) {
    ...
    if ((data[i] > data[i + 1]) || (data2[i] > data2[i + 1])) {
      pass = false;
      break;
    }
  }
