...
#define PROGRAM_NAME "decode_simple"
const int SUCCESS = 0;
const int FAILURE = -1;
...
int DecodeAndRenderFile(std::ifstream& stream) {
  ...
  vpl::Workstream decoder(VPL_TARGET_DEVICE_DEFAULT, VPL_WORKSTREAM_DECODE);
...
void PrintUsage(FILE* stream) {
  ...
  fprintf(stream,
          "Decode FILE using Intel(R) oneAPI Video Processing Library.\n\n"
...
void DisplayFrame(vplm_mem* image) {
  ...
  if (desc.format == VPLM_PIXEL_FORMAT_NV12) {
    cv::Mat img_nv12, img_bgra;
    unsigned char* ydata = data;
    unsigned char* uvdata = data + (desc.width * desc.height);
    ...
    for (size_t y = 0; y < desc.height / 2; y++) {
      memcpy(uvdata + (desc.width * y), handle.planes[1].data + (pitch1 * y),
             desc.width);
    }
    ...
    cv::waitKey(24);
  } else if (desc.format == VPLM_PIXEL_FORMAT_I420) {
    ...
    for (size_t y = 0; y < desc.height / 2; y++) {
      memcpy(udata + ((desc.width / 2) * y),
             handle.planes[1].data + (pitch1 * y), (desc.width / 2));
    }
    for (size_t y = 0; y < desc.height / 2; y++) {
      memcpy(vdata + ((desc.width / 2) * y),
             handle.planes[2].data + (pitch2 * y), (desc.width / 2));
    }
