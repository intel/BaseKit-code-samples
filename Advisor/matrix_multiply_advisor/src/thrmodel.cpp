...
#endif
{
  ...
  MULTIPLY(par->msize, par->tidx, par->numt, par->a, par->b, par->c, par->t);
#ifdef WIN32
  return 0;
#else
...
void ParallelMultiply(int msize, TYPE a[][NUM], TYPE b[][NUM], TYPE c[][NUM],
  ...
  for (tidx = 0; tidx < NTHREADS; tidx++) {
    ...
    par[tidx].t = t;
#ifdef WIN32
    ht[tidx] = (HANDLE)CreateThread(NULL, 0, ThreadFunction, &par[tidx], 0,
                                    &tid[tidx]);
#else
    tret[tidx] = pthread_create(&ht[tidx], NULL, (void*)ThreadFunction,
                                (void*)&par[tidx]);
#endif
  }
#ifdef WIN32
...
void GetModelParams(int* p_nthreads, int* p_msize, int print) {
  ...
  // of the matrix tails
  while ((msize % nthr) != 0) nthr--;
  // If kernel multiply0, set single threaded execution
  if (MULTIPLY == multiply0) nthr = 1;

  if (p_nthreads != 0) *p_nthreads = nthr;
  if (p_msize != 0) *p_msize = msize;
...
void ParallelMultiply(int msize, TYPE a[][NUM], TYPE b[][NUM], TYPE c[][NUM],
  ...
  if (strncmp(xstr(MULTIPLY), "multiply5", 16) != 0) {
    printf(
        "===== Error: Change matrix kernel to 'multiply5' for compilation with "
        "MKL =====\n");
    fflush(stdout);
    return;
  }
